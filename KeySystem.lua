...
-- ── KEY CACHE ─────────────────────────────────────────────────────────────
-- Saves: "KEY|TIMESTAMP" to a local file
-- On re-execute: reads file, checks if still valid, skips UI if good
...
local function loadCachedKey()
	local ok, content = pcall(function() return readfile(CACHE_FILE) end)
	if not ok or not content or content == "" then return nil end
	local key, timestamp = content:match("^(.+)|(%d+)$")
...
local function validateWithServer(key)
	local url = KEY_API .. "?action=validate&key=" .. key .. "&user=" .. player.Name
...
UserInputService.InputChanged:Connect(function(i) if dragging then onDragUpdate(i) end end)
...
CloseBtn.BackgroundColor3 = C.redBg; CloseBtn.Text = "✕"
CloseBtn.TextColor3 = C.red; CloseBtn.TextSize = 13
CloseBtn.Font = Enum.Font.GothamBold; CloseBtn.AutoButtonColor = false; CloseBtn.ZIndex = 13
Instance.new("UICorner", CloseBtn).CornerRadius = UDim.new(0, 5)
CloseBtn.MouseEnter:Connect(function()
	TweenService:Create(CloseBtn, TweenInfo.new(0.12), {BackgroundColor3=C.red, TextColor3=Color3.new(1,1,1)}):Play()
end)
CloseBtn.MouseLeave:Connect(function()
	TweenService:Create(CloseBtn, TweenInfo.new(0.12), {BackgroundColor3=C.redBg, TextColor3=C.red}):Play()
end)
...
TLine.Size = UDim2.new(1,-16,0,1); TLine.Position = UDim2.new(0,8,0,40)
TLine.BackgroundColor3 = C.accentDim; TLine.BorderSizePixel = 0; TLine.ZIndex = 11
...
LockIcon.Size = UDim2.new(1,0,0,46); LockIcon.Position = UDim2.new(0,0,0,48)
LockIcon.BackgroundTransparency = 1; LockIcon.Text = "🔒"; LockIcon.TextSize = 30
LockIcon.Font = Enum.Font.GothamBold; LockIcon.TextXAlignment = Enum.TextXAlignment.Center
LockIcon.ZIndex = 11

local InstLabel = Instance.new("TextLabel", Card)
InstLabel.Size = UDim2.new(1,-40,0,20); InstLabel.Position = UDim2.new(0,20,0,96)
InstLabel.BackgroundTransparency = 1; InstLabel.Text = "Enter your key to continue"
InstLabel.TextColor3 = C.textMid; InstLabel.TextSize = 14
InstLabel.Font = Enum.Font.Gotham; InstLabel.TextXAlignment = Enum.TextXAlignment.Center
InstLabel.ZIndex = 11

local InputBox = Instance.new("TextBox", Card)
InputBox.Size = UDim2.new(1,-40,0,36); InputBox.Position = UDim2.new(0,20,0,122)
InputBox.BackgroundColor3 = C.inputBg; InputBox.BackgroundTransparency = 0.20
InputBox.BorderSizePixel = 0; InputBox.Text = ""
InputBox.PlaceholderText = "Paste key here..."; InputBox.PlaceholderColor3 = C.textDim
InputBox.TextColor3 = C.textBright; InputBox.TextSize = 14
InputBox.Font = Enum.Font.Gotham; InputBox.ClearTextOnFocus = false; InputBox.ZIndex = 12
Instance.new("UICorner", InputBox).CornerRadius = UDim.new(0, 10)
local InputStroke = Instance.new("UIStroke", InputBox)
InputStroke.Color = C.border; InputStroke.Thickness = 1
local InputPad = Instance.new("UIPadding", InputBox)
InputPad.PaddingLeft = UDim.new(0,10); InputPad.PaddingRight = UDim.new(0,10)

local StatusLabel = Instance.new("TextLabel", Card)
StatusLabel.Size = UDim2.new(1,-40,0,18); StatusLabel.Position = UDim2.new(0,20,0,163)
StatusLabel.BackgroundTransparency = 1; StatusLabel.Text = ""
StatusLabel.TextColor3 = C.red; StatusLabel.TextSize = 13
...
SubmitBtn.Size = UDim2.new(1,-40,0,36); SubmitBtn.Position = UDim2.new(0,20,0,186)
SubmitBtn.BackgroundColor3 = C.accentDim; SubmitBtn.BorderSizePixel = 0
SubmitBtn.Text = "🔓  Unlock"; SubmitBtn.TextColor3 = C.textBright
SubmitBtn.TextSize = 15; SubmitBtn.Font = Enum.Font.GothamBold
SubmitBtn.AutoButtonColor = false; SubmitBtn.ZIndex = 12
...
LootStroke.Color = Color3.fromRGB(200,130,0); LootStroke.Thickness = 1
...
HintLabel.Size = UDim2.new(1,-40,0,16); HintLabel.Position = UDim2.new(0,20,0,272)
HintLabel.BackgroundTransparency = 1; HintLabel.Text = "Complete the offer to receive your key"
HintLabel.TextColor3 = C.textDim; HintLabel.TextSize = 11
HintLabel.Font = Enum.Font.Gotham; HintLabel.TextXAlignment = Enum.TextXAlignment.Center
HintLabel.ZIndex = 11

local ResetDivider = Instance.new("Frame", Card)
ResetDivider.Size = UDim2.new(1,-40,0,1); ResetDivider.Position = UDim2.new(0,20,0,296)
ResetDivider.BackgroundColor3 = C.border; ResetDivider.BorderSizePixel = 0; ResetDivider.ZIndex = 11

local ResetBox = Instance.new("TextBox", Card)
ResetBox.Size = UDim2.new(0,196,0,30); ResetBox.Position = UDim2.new(0,20,0,306)
ResetBox.BackgroundColor3 = C.inputBg; ResetBox.BackgroundTransparency = 0.20
ResetBox.BorderSizePixel = 0; ResetBox.Text = ""
ResetBox.PlaceholderText = "Reset code..."; ResetBox.PlaceholderColor3 = C.textDim
ResetBox.TextColor3 = C.textBright; ResetBox.TextSize = 13
ResetBox.Font = Enum.Font.Gotham; ResetBox.ClearTextOnFocus = false; ResetBox.ZIndex = 12
...
local RP = Instance.new("UIPadding", ResetBox); RP.PaddingLeft = UDim.new(0,8)

local ResetBtn = Instance.new("TextButton", Card)
ResetBtn.Size = UDim2.new(0,116,0,30); ResetBtn.Position = UDim2.new(0,224,0,306)
ResetBtn.BackgroundColor3 = Color3.fromRGB(30,50,70); ResetBtn.BorderSizePixel = 0
ResetBtn.Text = "Reset HWID"; ResetBtn.TextColor3 = Color3.fromRGB(150,200,255)
ResetBtn.TextSize = 13; ResetBtn.Font = Enum.Font.GothamBold
ResetBtn.AutoButtonColor = false; ResetBtn.ZIndex = 12
...
ResetBtn.MouseEnter:Connect(function()
	TweenService:Create(ResetBtn, TweenInfo.new(0.12), {BackgroundColor3=Color3.fromRGB(50,80,110)}):Play()
end)
ResetBtn.MouseLeave:Connect(function()
	TweenService:Create(ResetBtn, TweenInfo.new(0.12), {BackgroundColor3=Color3.fromRGB(30,50,70)}):Play()
end)
...
local function onSubmit()
	...
	SubmitBtn.Active = false
	task.spawn(function()
		local result = validateWithServer(entered)
		if result == "OK" then
...
ResetBtn.MouseButton1Click:Connect(function()
	...
	if code == "" then setStatus("⚠  Enter your reset code.", true) return end
	setStatus("⏳  Resetting HWID...", false)
	ResetBtn.Active = false
	task.spawn(function()
		local url = KEY_API .. "?action=reset&resetcode=" .. code .. "&user=" .. player.Name
		local ok, result = pcall(function() return game:HttpGet(url) end)
		if not ok then setStatus("✘  Network error.", true) ResetBtn.Active = true return end
		result = result:match("^%s*(.-)%s*$")
		if result == "RESET_OK" then
			setStatus("✔  HWID reset! Re-enter your key.", false)
			ResetBox.Text = ""
			pcall(function() delfile(CACHE_FILE) end) -- clear local cache too
